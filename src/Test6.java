...
public class Test6 {
	...
	public static String orderThem(String toOrder){
		
		String order[] = { " ", "a", "b", "c", "d", "e", "f", "g", "h", "i", "j", "k", "l", "m", "n", "o", "p", "q",
				"r", "s", "t", "u", "v", "w", "x", "y", "z" }; // length 26
		
		String separate[] = toOrder.split(",");
		
		int lettercnt = 0;
		
		String ind[] = new String[order.length];
		
		for(int x = 0; order.length-1 >= x; x++) {
			
			for( int y = 0; separate.length > y; y++) {
				
				if( separate[y].substring(0, 1).equals(order[x])) {
	...
	public static void main(String[] args) {
		...
		while (finishFlag) {
			...
			String finish = orderThem(allWords);
